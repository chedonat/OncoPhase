...
casestudy_list2<-list(
  "Case Study 1"=list(lambda_S=3,mu_S=10,major_cn=2,minor_cn=1,lambda_G=8, mu_G=5),
  "Case Study 2"=list(lambda_S=3,mu_S=25,major_cn=3,minor_cn=1,lambda_G=20, mu_G=8),
  "Case Study 3"=list(lambda_S=3,mu_S=5,major_cn=1,minor_cn=0,lambda_G=5, mu_G=3),
  "Case Study 4"=list(lambda_S=7,mu_S=8,major_cn=2,minor_cn=1,lambda_G=9, mu_G=6),
  "Case Study 5"=list(lambda_S=2,mu_S=6,major_cn=1,minor_cn=1,lambda_G=4, mu_G=4),
  "Case Study 6"=list(lambda_S=14,mu_S=10,major_cn=3,minor_cn=1,lambda_G=16, mu_G=8),
  "Case Study 7"=list(lambda_S=1,mu_S=7,major_cn=2,minor_cn=0, lambda_G=8, mu_G=0),
  "Case Study 8"=list(lambda_S=3,mu_S=5,major_cn=1,minor_cn=0, lambda_G=6, mu_G=2),
  "Case Study 9"=list(lambda_S=6,mu_S=2,major_cn=2,minor_cn=0, lambda_G=7, mu_G=1),
  "Case Study A"=list(lambda_S=6,mu_S=8,major_cn=2,minor_cn=1, lambda_G=8, mu_G=6),
  "Case Study B"=list(lambda_S=4,mu_S=8,major_cn=2,minor_cn=0, lambda_G=8, mu_G=4),
  "Case Study C"=list(lambda_S=6,mu_S=14,major_cn=2,minor_cn=1, lambda_G=8, mu_G=12)
)
...
#for(ics in 11:11 ){
  ...
  #cs0 = do.call(build_casestudy, cstudy[[ics]])
  prevalence0 = do.call(getPrevalence, cstudy[[ics]]) #(cs0$snp_allelecount_df, cs0$ref_allelecount_df, cs0$phasing_association_df, cs0$major_copynumber_df,cs0$minor_copynumber_df, method=mymethod)
  
  prevalence0= as.numeric(prevalence0)
 ...
 # next
  ...
  for ( di in 1:n_d ) {
    ...
    for ( si in 1:n_simulations ) {
      ...
      input_list=do.call(add_depthcoverage,append(cstudy[[ics]],list(depthOfCoverage=d_range[di])))
      prevalence=do.call(getPrevalence,input_list )
    ...
    #  }else{
     ...
     #   stop(1)
      ...
      if(!is.na(prevalence))
        ...
        if(prevalence>1.01 || prevalence< -0.01)
        stop()
      #print the result
